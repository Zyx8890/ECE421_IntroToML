...
Original file is located at
    https://colab.research.google.com/drive/1i5AYIoXJnM16zn2MLWOWe3zIPNpVoc8M
"""
...
def train_gmm(train_data, test_data, k = 5, epoch=1000, init_kmeans=False):
  ...
  for i in range(epoch):
    ...
    log_PDF = log_GaussPDF(X_train, mu, sigma) # <-- FILLED
    _, log_marginal = log_posterior(log_PDF, logpi) # <-- FILLED
...
#to upload the data2D.npy to google colab
from google.colab import files
uploaded = files.upload()

# running the experiment for different k and init methods
for k in [1,2,3,4,5]:
    plt.figure(figsize=(12,5))
    plt.subplot(1,2,1)
    test_GMM(k, init_kmeans=True)   # kmeans initialization
    plt.title(f'k={k}  (init_kmeans=True)')
    plt.subplot(1,2,2)
    test_GMM(k, init_kmeans=False)  # random initialization
    plt.title(f'k={k}  (init_kmeans=False)')
    plt.show()